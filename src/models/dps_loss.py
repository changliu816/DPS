...
def dps_loss(args, clip_encoder, classification_head, ranked_list, logger):
    assert args.train_dataset is not None, "Please provide a training dataset."
    logger.info('Fine-tuning Using DPS Loss')
    model = clip_encoder
    input_key = 'images'
    ...
    print('The masking percentage is: ', args.masking *100)
    frozen_list = ranked_list[: int( args.masking * len(ranked_list))]
    layer_channel_dict = {}
    # Iterate over the frozen_list
    for layer_num, channel_num, _ in frozen_list:
        # If layer_num is not in the dictionary, initialize it with an empty list
        if layer_num not in layer_channel_dict:
            layer_channel_dict[layer_num] = []
        # Append the channel_num to the corresponding layer_num list
        layer_channel_dict[layer_num].append(channel_num)

    clip_params = list(model.named_parameters())
    # Iterate through params and freeze parameters that match the frozen_set
    for layer_num, param in clip_params:
        # Check if the layer number is in the frozen set
        if layer_num in layer_channel_dict:  # Get all layer numbers from frozen_set
            indexes = layer_channel_dict[layer_num]
            param[indexes].requires_grad = False  # Freeze the specific channel

    params = [param for name, param in clip_params if param.requires_grad]
